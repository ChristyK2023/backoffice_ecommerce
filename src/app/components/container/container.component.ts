...
export class ContainerComponent implements OnInit, OnDestroy {
  ...
  initComp() {
    ...
    this.entityNames = localData
      ? localData?.entityNames
      : [this.entityNamesAll[0]];
  }
  ...
  getDatasByPage() {
    if (this.query) {
      this.searchDataByPage$ = this.entityService
        .searchDataByPage(
          this.entity,
          this.query,
          this.pageNumber,
      ...
      this.getDatasByPage$ = this.entityService
        .getDatasByPage(this.entity, this.pageNumber, this.pageLimit)
        .subscribe({
          next: (data: any) => {
            const { isSuccess, results } = data;
            if (isSuccess && results) {
  ...
  setEntityNames(event: any, name: String) {
    ...
    if (checked) {
      if (!this.entityNames.includes(name)) {
        ...
        oldValue.push(name);
        this.entityNames = [];
        this.entityNames = this.entityNamesAll.filter((name) =>
  ...
  async handleConfirmModal(event: any) {
    ...
    if (this.entityDelete) {
      this.entityService
        .deleteData(this.entity, this.entityDelete._id)
        .subscribe({
          next: (value: any) => {
            console.log(value);
            this.getDatasByPage();
